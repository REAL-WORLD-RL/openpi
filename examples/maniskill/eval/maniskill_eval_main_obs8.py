...
def _quat2axisangle(quat):
    """
    将四元数转换为轴角表示 (axis-angle representation)
    Copied from robosuite: https://github.com/ARISE-Initiative/robosuite/blob/eafb81f54ffc104f905ee48a16bb15f059176ad3/robosuite/utils/transform_utils.py#L490C1-L512C55
    
    Args:
        quat: 四元数 [qw, qx, qy, qz] - scalar-first格式
    ...
    return (quat[1:4] * 2.0 * math.acos(quat[0])) / den
...
def eval_maniskill(args: Args) -> None:
    ...
    # - obs_mode="rgb" provides RGB images + tcp_pose in extra + qpos in agent
    # - control_mode="pd_ee_delta_pose" (end-effector delta pose control)
    # - Training data uses 8-dim state: 3 eef_pos + 3 eef_rot (axis-angle) + 2 gripper
    ...
    # Check if extra contains tcp_pose (末端执行器位姿)
        ...
        if isinstance(test_obs['extra'], dict):
            ...
            for k, v in test_obs['extra'].items():
                if hasattr(v, 'shape'):
                    ...
                    # Special handling for tcp_pose
                    if k == 'tcp_pose':
                        tcp_pose_val = v.cpu().numpy() if hasattr(v, 'cpu') else np.array(v)
                        tcp_pose_val = tcp_pose_val.flatten()
                        logging.info(f"    tcp_pose format: [x, y, z, qw, qx, qy, qz]")
                        logging.info(f"    tcp_pose value: {tcp_pose_val}")
    ...
    for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
        ...
        while t < max_len:
            ...
            try:
                ...
                # State format: 8-dim = 3 eef_pos + 3 eef_rot (axis-angle) + 2 gripper
                # - [0:3]: eef_pos - 末端执行器位置 (x, y, z)
                # - [3:6]: eef_rot - 末端执行器旋转-轴角表示 (axis-angle)
                # - [6:8]: gripper - 夹爪位置 (left_finger, right_finger)
                
                # 1. Extract tcp_pose from extra (末端执行器位姿)
                # tcp_pose: [x, y, z, qw, qx, qy, qz] - 7维
                if "extra" not in obs or "tcp_pose" not in obs["extra"]:
                    raise ValueError(f"'tcp_pose' not found in obs['extra']! Keys: {list(obs.get('extra', {}).keys())}")
                
                tcp_pose = obs["extra"]["tcp_pose"]
                if hasattr(tcp_pose, 'cpu'):
                    tcp_pose = tcp_pose.cpu().numpy()
                tcp_pose = tcp_pose.flatten()
                
                # Extract position and orientation
                eef_pos = tcp_pose[:3]  # [x, y, z]
                eef_quat = tcp_pose[3:]  # [qw, qx, qy, qz]
                
                # Convert quaternion to axis-angle (3-dim)
                eef_rot_axis_angle = _quat2axisangle(eef_quat)
                
                # 2. Extract gripper positions from qpos
                if "agent" not in obs or "qpos" not in obs["agent"]:
                    raise ValueError(f"'qpos' not found in obs['agent']! Keys: {list(obs.get('agent', {}).keys())}")
                ...
                # qpos: [joint1, ..., joint7, left_finger, right_finger] - 9维
                # Extract last 2 dims for gripper
                if len(qpos) < 9:
                    raise ValueError(f"Expected qpos to have at least 9 dims, got {len(qpos)}")
                gripper_pos = qpos[7:9]  # [left_finger, right_finger]
                
                # 3. Combine into 8-dim state
                state = np.concatenate([
                    eef_pos,              # [0:3] 末端执行器位置
                    eef_rot_axis_angle,   # [3:6] 末端执行器旋转-轴角
                    gripper_pos           # [6:8] 夹爪位置
                ]).astype(np.float32)
                
                # Output state dimensions (only once)
                if t == 0 and episode_idx == 0:
                    logging.info(f"State dimension: {state.shape[0]} (3 eef_pos + 3 eef_rot + 2 gripper)")
                    logging.info(f"State format:")
                    logging.info(f"  eef_pos[0:3]={eef_pos}")
                    logging.info(f"  eef_rot[3:6]={eef_rot_axis_angle}")
                    logging.info(f"  gripper[6:8]={gripper_pos}")
