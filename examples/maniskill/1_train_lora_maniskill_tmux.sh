...
# Maniskill 训练脚本 - Tmux 版本
# 使用 GPU 4,5,6,7

# Tmux 会话名称
SESSION_NAME="maniskill_train"
...
tmux send-keys -t $SESSION_NAME "export HF_LEROBOT_HOME=/share_data/caiyishuai/openpi/data/maniskill" C-m
...
tmux send-keys -t $SESSION_NAME "echo '  Config: pi05_maniskill5_lora' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset: wps852/pushcube' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset Path: \$HF_LEROBOT_HOME/wps852/pushcube' " C-m
...
tmux send-keys -t $SESSION_NAME " echo '步骤2: 开始训练...' && uv run scripts/train.py pi05_maniskill5_lora --exp-name=pushcube --overwrite" C-m
