...
tmux send-keys -t $SESSION_NAME "export HF_LEROBOT_HOME=/share_data/caiyishuai/openpi/data/mujoco_franka" C-m
...
tmux send-keys -t $SESSION_NAME "echo '  Config: pi05_mujoco_franka_lora' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset: franka_lift_cube_success_trajs_100_lerobot' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset Path: \$HF_LEROBOT_HOME/franka_lift_cube_success_trajs_100_lerobot' " C-m
...
tmux send-keys -t $SESSION_NAME "uv run scripts/compute_norm_stats.py --config-name pi05_mujoco_franka_lora" C-m

# 等待第一步完成，然后开始训练
tmux send-keys -t $SESSION_NAME "echo '' " C-m
tmux send-keys -t $SESSION_NAME "echo '步骤2: 开始训练...' " C-m
tmux send-keys -t $SESSION_NAME "uv run scripts/train.py pi05_mujoco_franka_lora --exp-name pi05_libero_mujoco_franka_lora --overwrite" C-m
