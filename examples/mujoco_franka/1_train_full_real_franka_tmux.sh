...
# Mujoco Franka 训练脚本 - Tmux 版本
# 使用 GPU 4,5,6,7

# Tmux 会话名称
SESSION_NAME="mujoco_franka_train"
...
tmux send-keys -t $SESSION_NAME "export HF_LEROBOT_HOME=/share_data/caiyishuai/openpi/data/mujoco_franka" C-m # 注意：这里需要修改为实际的数据集路径
...
tmux send-keys -t $SESSION_NAME "echo '  Config: pi05_mujoco_franka' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset: franka_lift_cube_success_trajs_100_lerobot' " C-m # 注意：这里需要修改为实际的数据集路径
tmux send-keys -t $SESSION_NAME "echo '  Dataset Path: \$HF_LEROBOT_HOME/franka_lift_cube_success_trajs_100_lerobot' " C-m # 注意：这里需要修改为实际的数据集路径
...
tmux send-keys -t $SESSION_NAME "uv run scripts/compute_norm_stats.py --config-name pi05_mujoco_franka && echo '归一化统计完成!' || echo '归一化统计失败!'" C-m

# 等待第一步完成，然后开始训练
tmux send-keys -t $SESSION_NAME "echo '' " C-m
tmux send-keys -t $SESSION_NAME "echo '步骤2: 开始训练...' " C-m
tmux send-keys -t $SESSION_NAME "uv run scripts/train.py pi05_mujoco_franka --exp-name pi05_libero_mujoco_franka --overwrite" C-m
...
# export HF_LEROBOT_HOME=/share_data/caiyishuai/openpi/data/mujoco_franka
