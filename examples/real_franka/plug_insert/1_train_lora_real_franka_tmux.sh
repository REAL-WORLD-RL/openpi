...
tmux send-keys -t $SESSION_NAME "export HF_LEROBOT_HOME=/share_data/caiyishuai/openpi/data/real_franka" C-m
...
tmux send-keys -t $SESSION_NAME "echo '  Config: real_plug_insert_pi05_libero_lora' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset: plug_insert_100_demos_lerobot' " C-m
tmux send-keys -t $SESSION_NAME "echo '  Dataset Path: \$HF_LEROBOT_HOME/plug_insert_100_demos_lerobot' " C-m
...
# 步骤1: 计算归一化统计数据，然后开始训练
tmux send-keys -t $SESSION_NAME "echo '' " C-m
# tmux send-keys -t $SESSION_NAME "echo '步骤1: 计算归一化统计数据...' " C-m
# tmux send-keys -t $SESSION_NAME "uv run scripts/compute_norm_stats.py --config-name real_plug_insert_pi05_libero_lora && echo '' && echo '步骤2: 开始训练...' && uv run scripts/train.py real_plug_insert_pi05_libero_lora --exp-name lora_libero_real_franka_plug_insert_100demos --overwrite" C-m
tmux send-keys -t $SESSION_NAME " echo '步骤2: 开始训练...' && uv run scripts/train.py real_plug_insert_pi05_libero_lora --exp-name real_plug_insert_libero_lora_100demos --overwrite" C-m
...
# tmux kill-session -t franka_train
